...
def main():
    ...
    if RUN_UCS or RUN_ALL:
        print(f" **** RUNNING UCS ******")
        Solver(initial_board, HEURISTIC.H0_PURELY_COST_FOR_UCS, ALGORITHM.UCS).run()
    if RUN_GBFS_H1 or RUN_ALL:
        print(f" **** RUNNING GBFS on H1 ******")
        Solver(initial_board, HEURISTIC.H1_NUMBER_BLOCKING_VEHICLES, ALGORITHM.GBFS).run()
    if RUN_GBFS_H2 or RUN_ALL:
        print(f" **** RUNNING GBFS on H2 ******")
        Solver(initial_board, HEURISTIC.H2_NUMBER_BLOCKED_POSITIONS, ALGORITHM.GBFS).run()
    if RUN_GBFS_H3 or RUN_ALL:
        print(f" **** RUNNING GBFS on H3 ******")
        Solver(initial_board, HEURISTIC.H3_H1_TIMES_LAMBDA, ALGORITHM.GBFS, LAMBDA).run()
    if RUN_GBFS_H4 or RUN_ALL:
        print(f" **** RUNNING GBFS on H4 ******")
        Solver(initial_board, HEURISTIC.H4_CUSTOM, ALGORITHM.GBFS).run()
    if RUN_A_H1 or RUN_ALL:
        print(f" **** RUNNING A/A* on H1 ******")
        Solver(initial_board, HEURISTIC.H1_NUMBER_BLOCKING_VEHICLES, ALGORITHM.A).run()
    if RUN_A_H2 or RUN_ALL:
        print(f" **** RUNNING A/A* on H2 ******")
        Solver(initial_board, HEURISTIC.H2_NUMBER_BLOCKED_POSITIONS, ALGORITHM.A).run()
    if RUN_A_H3 or RUN_ALL:
        print(f" **** RUNNING A/A* on H3 ******")
        Solver(initial_board, HEURISTIC.H3_H1_TIMES_LAMBDA, ALGORITHM.A, LAMBDA).run()
    if RUN_A_H4 or RUN_ALL:
        print(f" **** RUNNING A/A* on H4 ******")
        Solver(initial_board, HEURISTIC.H4_CUSTOM, ALGORITHM.A).run()
